...
<objective>
Install Phase 2 dependencies (motion, recharts, lucide-react, cva, clsx, tailwind-merge), create UI primitives (cn utility, Button component), build the animated Hero landing page with cycling text, and set up separate route routing (/ = landing, /session = detection).
...
<task type="auto">
  ...
  <action>
    1. Install Phase 2 dependencies:
       ...
       ```
       Note: Install `motion` (NOT `framer-motion`) -- it is the rebranded package. Import from `motion/react`.
  ...
  <verify>
    - `npm ls motion recharts lucide-react class-variance-authority clsx tailwind-merge` shows all installed
...
<task type="auto">
  ...
  <action>
    1. Create `src/components/landing/Hero.tsx` as a 'use client' component:
       - Cycling words animation using AnimatePresence from `motion/react` with mode="wait"
       ...
       - Add fade-in animation on the entire hero section using motion.div (initial opacity:0, animate opacity:1)
